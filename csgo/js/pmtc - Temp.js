...
$(document).ready(function () {
	...
	*/
	$("#tabList li.btn-game").click(function () {
		...
		$("#tabList li").removeClass('selected');
		$(this).addClass('selected');
		$("#panels .main, #panels #output-tab, #panels .veto-tab").css("display", "none");
	...
	$("#tabList li#btn-output").click(function () {
		$("#tabList li").removeClass('selected');
		$(this).addClass('selected');
		$("#panels .main, #panels .veto-tab").css("display", "none");
		$("#panels #output-tab").css("display", "inline-block");
		
		
		// $.post("../csgo/downloadHTML.php", {
			// feed: "https://liquipedia.net/counterstrike/BLAST/Premier/2021/World_Final"
		// }).done(function (d) {
			// console.log(JSON.stringify(d));
			
			// var lines = d.split('],['); //array of lines in hltv's html file
			...
			// for (i in lines) {
				...
				// if (line.indexOf("standard-box veto-box") !== -1 && lines[parseInt(i) + 2].indexOf('1.') !== -1) {
					// console.log(lines[parseInt(i) + 2]);
					// console.log(lines[parseInt(i) + 3]);
					// console.log(lines[parseInt(i) + 4]);
					// console.log(lines[parseInt(i) + 5]);
					// console.log(lines[parseInt(i) + 6]);
					// console.log(lines[parseInt(i) + 7]);
					// console.log(lines[parseInt(i) + 8]);
				// }
				
				// if (line.indexOf("results-team-score") !== -1) {
					// console.log(line);
				// }
				
				// if (line.indexOf("results-center-stats") !== -1) {
					// console.log(line);
					// for (i = 73; line[i] !== "\""; i++) {
						// mapLinks[mapNum] += line[i];
					// }
					// mapLinks[mapNum].replace("undefined", "");
					// for (i = 26; line[i] !== "\/"; i++) {
						// mapIDs[mapNum] += mapLinks[mapNum][i];
					// }
					// mapIDs[mapNum].replace("undefined", "");
					// console.log(mapLinks[mapNum]);
					// console.log(mapIDs[mapNum]);
					// mapNum++;
				// }
		...
		//read html file of hltv page
		//filePath = "../csgo/los-grandes-vs-leviatan-esl-challenger-anaheim-2022-south-america-closed-qualifier"
		//var file = null;
		...
		//"../csgo/los-grandes-vs-leviatan-esl-challenger-anaheim-2022-south-america-closed-qualifier"
			...
			if (team1ID != "" && team2ID != "") {
				...
				$("#panels > div:nth-child(" + (i + 1) + ") [o]").each(function () {
					...
					if ( (index != 6) && (index != 12) ) { //don't include extra player on each team (players 11 and 12 = indexes 6 and 12)
						...
						data[i]["P" + playerNumber].value = $(this).val();
		...
		for (i = 1; i < data.length; i++) {
			...
			if (!($("#panels > div:nth-child(" + (i + 1) + ") .T1").val() == null || $("#panels > div:nth-child(" + (i + 1) + ") .T1").val() == '')) {
				...
				if (['1', '2', '3', '4'].indexOf($("#panels > div:nth-child(" + (i + 1) + ") .PNum").val()) > -1) {
					...
					main += $('textarea#main-scoreboard-stats-' + numP + 'v' + numP).val();
				} else {					
					main += $('textarea#main-scoreboard-stats').val();
				}
			}
			for (j in data[i]) {
...
function IconifyVeto(teamID) {
	if (TEAMS[teamID]["LOGO"].indexOf("lang") == -1) //regular logo
		return "[" + Initialsa(teamID) + "](#" + TEAMS[teamID]["LOGO"] + "-logo)";
	else 									//use initials instead
...
function IconifyVetoW(teamID) {
	if (TEAMS[teamID]["LOGOW"] == "FALSE")
		...
		return "[" + Initialsa(teamID) + "](#" + TEAMS[teamID]["LOGO"] + "w-logo)";
}
